...
function gameOver() {
    ...
    fetch('/score', {
        ...
        body: JSON.stringify({ score: gameState.wave }) // Envia a onda alcançada como pontuação
    })
    .then(response => console.log("Pontuação enviada ao banco de dados Express com sucesso."))
    .catch(error => console.error("Falha ao comunicar com o endpoint Ajax:", error));
}
